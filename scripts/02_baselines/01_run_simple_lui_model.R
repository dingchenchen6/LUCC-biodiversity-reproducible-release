...
Sites <- readRDS(file = paste0(inDir,"PREDICTSSiteData1.rds")) # 6462 rows
sites <- readRDS(file = paste0(inDir,"PREDICTSSiteData.rds")) # 6462 rows
...
model_data_sr <- na.omit(Sites[,c('Species_richness','LandUse','Use_intensity','UI2','SS','SSB','SSBS','UN_region')])
model_data_sr$UI2 <- factor(model_data_sr$UI2,
...
# summaries
length(unique(model_data_sr$SS)) # 95/138
length(unique(model_data_sr$SSBS)) # 3783/6462


# look at the spread of land use/use intensity categories
print(table(model_data_sr$UI2))

# Agriculture_High    Agriculture_Low   Primary vegetation Secondary vegetation Urban
# 1297                 999               2517              1211                 438
...
sm3_1 <- GLMER(modelData = model_data_sr,responseVar = "Species_richness",fitFamily = "poisson",
             fixedStruct = "UI2*UN_region",randomStruct = "(1|SS)+(1|SSB)+(1|SSBS)",REML = FALSE)
sm3$model
...
# df      AIC
# sm0$model  4 38771.53
# sm1$model 12 38496.41
# sm2$model 14 38492.17
# sm3$model  8 38573.85
# sm4$model 27 38370.20
...
model_data_ab <- na.omit(Sites[,c('LogAbund','LandUse','Use_intensity','UI2','SS','SSB','SSBS','UN_region')])
model_data_ab$UI2 <- factor(model_data_ab$UI2,
  levels = c("Primary vegetation",
             "Secondary vegetation",
             "Agriculture_Low",
             "Agriculture_High",
...
# summaries
length(unique(model_data_ab$SS)) # 84/121
length(unique(model_data_ab$SSBS)) # 3380/5745/5817

# look at the spread of land use/use intensity categories
print(table(model_data_ab$UI2))

# Agriculture_High      Agriculture_Low   Primary vegetation Secondary vegetation  Urban 
# 1282                  854               2286               1034                  289 
...
am3_1 <- GLMER(modelData = model_data_ab,responseVar = "LogAbund",fitFamily = "gaussian",
             fixedStruct = "UI2*UN_region",randomStruct = "(1|SS)+(1|SSB)",REML = FALSE)
# 设置 UN_region 的对比方式为 sum-to-zero coding
unique(model_data_ab$UN_region)
model_data_ab$UN_region <- factor(model_data_ab$UN_region)
contrasts(model_data_ab$UN_region) <- contr.sum
am3_2 <- GLMER(
  modelData = model_data_ab,
  responseVar = "LogAbund",
  fitFamily = "gaussian",
  fixedStruct = "UI2 * UN_region",
...
library(emmeans)
emmeans(am3_2$model, pairwise ~ UI2 | UN_region)
...
# df      AIC
# am0$model  4 14299.77
# am1$model 12 14270.89
# am2$model 14 14262.75
# am3$model  8 14269.78
# am4$model 27 14243.05
...
abun_res$LU <- factor(c("Primary vegetation","Secondary vegetation", "Agriculture_Low","Agriculture_High","Urban"), levels = c("Primary vegetation","Secondary vegetation", "Agriculture_Low","Agriculture_High","Urban"))
rich_res$LU <- factor(c("Primary vegetation","Secondary vegetation", "Agriculture_Low","Agriculture_High","Urban"), levels = c("Primary vegetation","Secondary vegetation", "Agriculture_Low","Agriculture_High","Urban"))
...
p2 <- ggplot(data = abun_res) +
      ...
      scale_color_manual(values = c("#009E73","#0072B2","#E69F00","#D55E00","pink")) +
  ylim(c(-40, 60)) +
...
ggsave(filename = paste0(outDir, "Figure1_map_simplemods2.pdf"), plot = last_plot(), width = 120, height = 150, units = "mm", dpi = 300)

library(export)

graph2ppt(
  x = p5,
  file = paste0(outDir, "Figure1_map_simplemods2.pptx"),
  width = 12,
  height = 15
)
...
print(ppt, target = paste0(outDir, "Figure1_map_simplemods2.pptx"))
